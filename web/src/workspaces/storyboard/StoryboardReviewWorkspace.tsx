...
export type StoryboardPreviewMode = "segment" | "incoming" | "outgoing";
  ...
  return <section className="review-workspace" aria-label="分鏡審核工作區">
    <div className="review-browser">
      ...
      <div className="review-groups">
        {model.groups.map((group) => <section className="review-group" key={group.id}>
          <header>
            <div><b>{group.title}</b><small>{group.segments.length} 個片段 · {formatDuration(group.segments.filter((segment) => segment.included).reduce((total, segment) => total + segment.durationSeconds, 0))}</small></div>
...
function SegmentRow({ segment, index, selected, onSelect }: {
  ...
  >
    <span className="review-drag" aria-hidden="true">⋮⋮</span>
...
function SegmentInspector({
  ...
  return <aside className="review-inspector" aria-label="片段設定">
    ...
    <div className="review-preview-actions">
      <button type="button" disabled={previewing} onClick={() => onPreview(segment.id, "incoming")}>預覽前段銜接</button>
      <button type="button" disabled={previewing} onClick={() => onPreview(segment.id, "outgoing")}>預覽後段銜接</button>
