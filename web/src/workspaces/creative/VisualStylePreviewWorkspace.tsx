...
export function VisualStylePreviewWorkspace({ detail, setMessage, refreshProject, mutationControls }: Props) {
  ...
  async function approve() {
    ...
    setBusy(true);
    try {
      const result = await api.approveVisualStyle(detail.project.id, selected, detail.project_revision);
  ...
  return <section className="visual-style-preview card" aria-label="Visual Style Preview">
    ...
    <div className="visual-style-actions"><button type="button" disabled={!approvedBrief || busy} onClick={() => void preview()}>{busy ? "處理中…" : "產生真實畫面預覽"}</button>{approvedBrief && <><select value={selected} disabled={busy} onChange={(event) => setSelected(event.target.value)}>{styles.map((style) => <option key={String(style.style_id)} value={String(style.style_id)}>{String(style.label || style.style_id)}</option>)}</select><button type="button" className="primary" disabled={busy || !selected} onClick={() => void approve()}>核准選定 Visual Style</button></>}</div>
    {variants.length > 0 && <div className="visual-style-grid">{variants.map((variant) => { const style = (variant.visual_style || {}) as Record<string, unknown>; const source = (variant.source || {}) as Record<string, unknown>; return <article key={String(style.visual_style_id)}><img src={String(variant.url || "")} alt={`${String(style.label || style.visual_style_id)} true frame preview`} /><h4>{String(style.label || style.visual_style_id)}</h4><p>{String(style.composition || "overlay")} · grading {String((style.grading as Record<string, unknown>)?.look_id || "unknown")} · source {String(source.project_media_uuid || "")}</p><code>{String(style.resolved_hash || "").slice(0, 16)}</code></article>; })}</div>}
  </section>;
}
