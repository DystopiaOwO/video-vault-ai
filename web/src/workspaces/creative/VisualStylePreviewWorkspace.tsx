...
export function VisualStylePreviewWorkspace({ detail, setMessage, refreshProject, mutationControls, compact = false, showAdvanced = false, onApproved }: Props) {
  ...
  function selectVariant(variant: Record<string, unknown>, styleId?: string) {
    const style = mapValue(variant.visual_style);
    setSelected(String(styleId || style.visual_style_id || selected));
    setSelectedPreviewPlanHash(String(variant.preview_plan_hash || ""));
    setSelectedPreviewVariantId(String(variant.preview_variant_id || ""));
    setSelectedTitleRole(String(variant.title_role || mapValue(variant.representative_frame).title_role || "chapter_title"));
  }
  ...
  return <section className={`visual-style-preview card${compact ? " visual-style-preview-simple" : ""}`} aria-label="Visual Style Preview">
    ...
    <div className="visual-style-actions"><button type="button" disabled={!approvedBrief || busy} onClick={() => void preview()}>{busy ? "處理中…" : "產生真實畫面預覽"}</button>{approvedBrief && <><select className={compact ? "visual-style-hidden-picker" : ""} aria-label="選擇視覺風格" value={selected} disabled={busy} onChange={(event) => { const next = event.target.value; const match = heroFor(next); if (match) selectVariant(match, next); else { setSelected(next); invalidatePreview(); } }}>{styles.map((style) => <option key={String(style.style_id)} value={String(style.style_id)}>{String(style.label || style.style_id)}</option>)}</select><button type="button" className="primary" disabled={busy || !selected || !selectedPreviewPlanHash || !selectedPreviewVariantId} onClick={() => void approve()}>{compact ? "使用這個風格並繼續" : "核准選定 Visual Style"}</button></>}</div>
    {approvedBrief && (!compact || showAdvanced) && <div className="visual-style-overrides-shell" aria-label="視覺風格詳細設定"><div className="visual-style-overrides" aria-label="bounded visual style overrides">
