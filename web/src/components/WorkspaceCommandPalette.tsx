...
export function buildWorkspaceCommands(close: () => void): CommandAction[] {
  ...
  return [
    action("project-search", "搜尋專案", "將游標移到左側專案搜尋", "project search 專案 查找", () => document.querySelector<HTMLInputElement>('[aria-label="搜尋專案"]')?.focus()),
    action("overview", "前往總覽", "Render Job、進度與工作流", "overview jobs status 總覽 進度", () => scrollTo("#workspace-overview")),
    action("storyboard", "前往分鏡", "片段排序、剪點與 Inspector", "storyboard timing inspector 分鏡 剪點", () => scrollTo("#workspace-storyboard")),
    action("review", "前往審核與輸出", "核准、退回與正式輸出", "review render export 審核 輸出", () => scrollTo("#workspace-review")),
    action("media", "前往素材與調色", "素材感知、描述與色彩一致性", "media clip color 素材 調色 感知", () => scrollTo("#workspace-media")),
    action("audio", "前往音訊", "BGM、原音與片段例外", "audio bgm sound 音訊 混音", () => scrollTo("#workspace-audio")),
    action("script", "前往故事整理", "檢視專案敘事與腳本", "story script 故事 腳本", () => scrollTo("#workspace-script")),
...
export function WorkspaceCommandPalette() {
  ...
  return <>
    <button className="command-palette-trigger" type="button" onClick={() => setOpen(true)} aria-label="開啟命令面板">
    ...
    {open && <div className="command-palette-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section className="command-palette" role="dialog" aria-modal="true" aria-label="工作區命令面板">
        ...
        <div className="command-palette-results" role="listbox" aria-label="命令結果">
