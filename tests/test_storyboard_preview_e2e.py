1Îa°—1qÍmäâÀäw∂{g© ◊±∂◊±µ©ïßr´◊úÖ™ı—»ZØ]jyËµ 'ΩÍÌj∏.ôÈÌ¢WBÖ™äÿoj[û~ä—zôZqÎh∑*^K+-z`°j∞öûz-râÔzªoj[û∂ãr•‰≤≤◊¶
´∫Ë≠+käx&∫À[yÏZrŸr¢wúÖ™⁄r◊´ñâ‡import json
...
def _project(tmp_path: Path) -> tuple[dict, Path, int]:
    ...
    add_analysis(db, video_id, "mock", "rules", {"segments": [
        {"start_seconds": 0, "end_seconds": 5, "segment_type": "key_action", "title": "ÊäµÈÅî", "reason": "first", "tags": ["travel"], "score": 0.9, "suggested_use": "main"},
        {"start_seconds": 5, "end_seconds": 10, "segment_type": "detail", "title": "Áî®È§ê", "reason": "second", "tags": ["food"], "score": 0.8, "suggested_use": "B-roll"},
    ]}, tmp_path / "raw.json")
