...
def test_real_ffmpeg_delivery_qa_creates_evidence_without_touching_source(tmp_path: Path):
    ...
    output.with_name(output.name + ".render.json").write_text(json.dumps({"manifest_hash": manifest_hash, "output_sha256": _sha(output), "loudness": {"final": {"measured_I": -14, "measured_TP": -1.2}}, "qc": {"passed": True}, "measurements": {"decode": {"ok": True}, "timestamp_monotonic": True}}), encoding="utf-8")
