...
from video_vault.audio_preview import AudioPreviewError, audio_preview_file_path, render_project_audio_preview
...
def _project(tmp_path: Path) -> tuple[dict, Path, int, Path, Path]:
    ...
    _make_source(source)
    _make_bgm(bgm)
    video_id = upsert_video(db, {"original_path": str(source), "current_path": str(source), "filename": source.name, "category": "travel", "duration_seconds": 4})
    add_analysis(db, video_id, "mock", "audio-preview-e2e", {"segments": [{"start_seconds": 0, "end_seconds": 4, "segment_type": "key_action", "title": "preview", "reason": "e2e", "tags": ["travel"], "score": 1, "suggested_use": "main"}]}, tmp_path / "raw.json")
    project_id = create_project(db, "Audio Preview E2E", [video_id], category="travel")
